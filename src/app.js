...
function resultQuestionHtml(question, prefix = "all") {
  ...
  return `<article id="${resultQuestionAnchor(question.id, prefix)}" class="answer-review ${question.correct === true ? "correct" : question.correct === false ? "wrong" : ""}"><h3>Question ${question.position} · ${formatScore(question.earnedMarks)} / ${formatScore(question.marks)} marks</h3>${question.image ? `<img class="question-preview-image" src="${escapeHtml(question.image)}" alt="Question image" />` : ""}<div class="rich-content">${contentHtml(question.text)}</div><div class="result-options">${answers.map((answer, index) => `<div class="result-option ${hasSelection && Number(selected) === index ? "selected" : ""} ${correctIndex === index ? "correct" : ""}"><strong>(${letterLabels[index]})</strong><span>${contentHtml(answer)}</span>${hasSelection && Number(selected) === index ? `<em>Selected</em>` : ""}${correctIndex === index ? `<em>Correct</em>` : ""}</div>`).join("")}</div><div class="exam-meta"><span>Selected option: ${hasSelection ? `(${letterLabels[Number(selected)]}) ${contentHtml(answers[Number(selected)] || "")}` : "Not answered"}</span><span>Correct option: (${letterLabels[correctIndex]}) ${contentHtml(answers[correctIndex] || "")}</span><span>${question.correct === null ? "Skipped" : question.correct ? "Correct" : "Wrong"}</span></div><div class="explanation-box"><strong>Explanation</strong>${question.explanation ? `<div class="explanation-body rich-content">${contentHtml(question.explanation)}</div>` : `<p>No explanation has been added for this question yet.</p>`}${question.explanationImage ? `<img class="question-preview-image" src="${escapeHtml(question.explanationImage)}" alt="Explanation image" />` : ""}</div></article>`;
}
