...
function showStudentSettings(message = "", section = activeSettingsSection) {
  ...
  const content = `<div class="settings-split-layout"><aside class="settings-section-nav"><div class="settings-section-nav-scroll"><h3>Account</h3>${settingsNavButton("profile", "circle-user-round", "Profile")}${desktop ? settingsNavButton("updates", "download", "Updates") : ""}<h3>Help &amp; security</h3>${settingsNavButton("support", "circle-x", "Report a bug")}${admin ? settingsNavButton("admin", "badge-check", "Admin & 2FA") : ""}</div><div class="settings-account-card"><span class="settings-account-avatar">${studentAvatarMarkup(profile)}</span><span><b>${escapeHtml(displayName())}</b><small>${escapeHtml(email)}</small></span><em>${admin ? "Admin" : "Student"}</em></div></aside><div class="settings-detail">${profileSection}${updatesSection}${supportSection}${adminSection}</div></div>`;
