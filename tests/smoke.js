...
async function studentFlow() {
  ...
  const pricingRoot = window.document.querySelector(".pricing-app");
  assert.ok(pricingRoot);
  assert.equal(pricingRoot.querySelectorAll(".cx-price-card").length, 3);
  assert.equal(window.document.querySelector(".pricing-plan"), null);
  assert.match(pricingRoot.textContent, /PAST \+ 3/);
  ...
  assert.match(pricingRoot.querySelector('[data-plan-id="past-plus-3"] [data-subject-feats]').textContent, /2 subjects/);
  click(window, '[data-pricing-plan="past-plus-3"]');
